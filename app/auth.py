...
def require_api_key(api_key_header_value: str = Security(api_key_header)):
    if not Config.API_KEY:
        raise HTTPException(status_code=HTTP_401_UNAUTHORIZED, detail="API Key not configured on server")
...
def require_admin_key(x_admin_key: str = Header(..., alias="x-admin-key")):
    if not Config.ADMIN_API_KEY:
        raise HTTPException(status_code=HTTP_401_UNAUTHORIZED, detail="Admin API Key not configured on server")
